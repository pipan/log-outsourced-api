...
Switch to newly creted directory `cd log` and start app setup `php artisan setup` and database setup `php artisan setup:database`. This setup is interactive, so follow the instructions and fill in required values. Application will need access to one directory, so you will have to run `sudo chown -R www-data:www-data storage`
